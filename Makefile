...
# Versao da release (editar aqui antes de make release)
RELEASE_VERSION ?= 1.0.0

release:
	@echo "$(BLUE)[TCC]$(NC) Publicando release v$(RELEASE_VERSION)..."
	@if git rev-parse "v$(RELEASE_VERSION)" >/dev/null 2>&1; then \
		echo "$(YELLOW)[AVISO]$(NC) Tag v$(RELEASE_VERSION) ja existe localmente."; \
		exit 1; \
	fi
	@grep -q "version = \"$(RELEASE_VERSION)\"" pyproject.toml || \
		(echo "$(YELLOW)[AVISO]$(NC) Atualize version em pyproject.toml para $(RELEASE_VERSION)"; exit 1)
	@grep -q "## \[$(RELEASE_VERSION)\]" CHANGELOG.md || \
		(echo "$(YELLOW)[AVISO]$(NC) Adicione secao ## [$(RELEASE_VERSION)] em CHANGELOG.md"; exit 1)
	git tag -a "v$(RELEASE_VERSION)" -m "Release v$(RELEASE_VERSION)"
	git push origin "v$(RELEASE_VERSION)"
	@echo "$(GREEN)[SUCESSO]$(NC) Tag v$(RELEASE_VERSION) enviada. Acompanhe em Actions > Release."
...
help:
	...
	@echo "  make release   - Criar tag vX.Y.Z e publicar GitHub Release"
	@echo ""

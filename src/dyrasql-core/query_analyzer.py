...
class QueryAnalyzer:
    ...
    def _normalize_query_with_catalog(self, query: str) -> str:
        ...
        if not modified:
            ...
            query_normalized = re.sub(
                simple_pattern, add_catalog, query_normalized, flags=re.IGNORECASE
            )

        if modified:
            logger.info(
                f"Query normalizada com catálogo iceberg: {query[:100]}... -> {query_normalized[:100]}..."
            )
        ...
        try:
            ...
            explain_data = {
                ...
                "summary": {
                    ...
                    "total_size_bytes": parsed_result.get("total_size_bytes", 0)
                    if parsed_result
                    else 0,
                    "total_size_gb": (parsed_result.get("total_size_bytes", 0) / (1024**3))
                    if parsed_result
                    else 0,
                    "total_rows": parsed_result.get("total_rows", 0) if parsed_result else 0,
                    "total_cpu_cost": parsed_result.get("total_cpu_cost", 0)
                    if parsed_result
                    else 0,
                },
            ...
            if parsed_result:
                logger.info(
                    f"  Resumo: {explain_data['summary']['total_tables']} tabelas, "
                    f"{explain_data['summary']['total_size_gb']:.2f} GB, "
                    f"{explain_data['summary']['total_rows']:,.0f} linhas"
                )
            else:
                logger.info(
                    f"  EXPLAIN salvo com erro: {explain_result.get('error', 'Unknown error')}"
                )

        except Exception as e:
            logger.error(f"Erro ao salvar EXPLAIN: {str(e)}", exc_info=True)

    def _execute_trino_query(self, query: str) -> Dict[str, Any] | None:
        """
        ...
        try:
            response = requests.post(
                ...
                headers={"Content-Type": "text/plain", "X-Trino-User": self.trino_user},
                data=query,
                timeout=60,
            )
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"Erro ao executar query no Trino: {response.status_code} - {response.text}"
                )
            ...
            if "error" in result:
                ...
                return {"error": error_msg, "error_details": result.get("error"), "result": result}
            ...
            while next_uri:
                next_response = requests.get(
                    next_uri, headers={"X-Trino-User": self.trino_user}, timeout=60
                )
    ...
    def _parse_explain_io(self, explain_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for table_info in input_tables:
            ...
            if catalog and schema and table:
                ...
                logger.info(
                    f"  Tamanho estimado: {estimated_size_bytes:,.0f} bytes ({estimated_size_bytes / (1024**3):.2f} GB)"
                )
    ...
    def explain_io(self, query: str) -> Dict[str, Any] | None:
        """
        ...
        if result and result.get("error"):
            ...
            logger.info("Tentando salvar EXPLAIN com erro...")
        ...
        if not result or not result.get("data"):
            ...
            logger.info("Tentando salvar EXPLAIN sem dados...")
        ...
        try:
            ...
            if explain_json_str:
                ...
                logger.debug(
                    f"EXPLAIN parseado com sucesso: {len(explain_json.get('inputTableColumnInfos', []))} tabelas encontradas"
                )
                logger.info(
                    f"Salvando EXPLAIN parseado com {len(explain_json.get('inputTableColumnInfos', []))} tabelas..."
                )
                ...
                logger.info(
                    f"Chamando _save_explain com parsed_result: {parsed_result is not None}"
                )
                self._save_explain(
                    query,
                    {
                ...
                self._save_explain(
                    query,
                    {"raw": {}, "result_complete": result, "error": "Empty explain_json_str"},
                    None,
                    normalized_query,
                )
            ...
            logger.error(
                f"String recebida: {explain_json_str[:200] if explain_json_str else 'None'}..."
            )
    ...
    def analyze_query_io(self, query: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Análise de I/O concluída: {len(tables_metadata)} tabelas, {total_size:,.0f} bytes ({total_size / (1024**3):.2f} GB), {total_rows:,.0f} linhas"
        )
    ...
    def _extract_where_clause(self, query: str) -> str | None:
        """
        ...
        where_match = re.search(
            r"\bwhere\s+(.+?)(?:\s+group\s+by|\s+order\s+by|\s+limit|$)",
            query_lower,
    ...
    def analyze_complexity(self, query):
        ...
        aggregations = sum(
            len(list(projection.find_all(exp.AggFunc))) for projection in tree.expressions
        )

...
async def route_query(request_data: RouteRequest):
    ...
    try:
        ...
        logger.info(f"Metadados de I/O obtidos: {len(metadata)} tabelas, {io_analysis.get('total_size_bytes', 0):,.0f} bytes ({total_size_gb:.2f} GB) totais")
    ...
    try:
        ...
        if is_keepalive:
            ...
            if cached_decision:
                ...
                logger.info(f"   Score: {score:.3f} | Factors: volume={factors.get('volume', 0):.3f}, complexity={factors.get('complexity', 0):.3f}, historical={factors.get('historical', 0):.3f}")
                ...
                if is_metadata_query:
                    cluster_name = 'small'
                    logger.info("METADATA: Metadata query detected, using default cluster (small) without full analysis")
                    logger.info(f"   Cluster: {cluster_name}")
                    ...
                    logger.info(f"NEW ANALYSIS: Query analyzed from scratch (not in cache)")
                    ...
                    logger.info(f"   Cluster selected: {cluster_name} (analysis on small, execution on {cluster_name})")
        ...
        logger.info(f"EXECUTING: Executing query on cluster {cluster_name} with timeout of {timeout}s")
        ...
        for cluster_name_sub, cluster_url in [
    ...
    try:
        ...
        if query_id_match:
            ...
            if query_id in query_cluster_map:
                ...
                logger.info(f"PROXY: Using cluster '{cluster_name}' for query ID {query_id} (nextUri)")
            else:
                logger.warning(f"WARNING: Query ID {query_id} not found in mapping, using default cluster (small)")
        ...
        if request.method == 'GET':
            response = requests.get(target_url, headers=headers, params=dict(request.query_params), timeout=300)
        ...
        for cluster_name_sub, rewrite_url in [
            ('small', get_cluster_url('small')),
            ('medium', get_cluster_url('medium')),
            ('large', get_cluster_url('large'))
        ]:

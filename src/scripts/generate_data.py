...
def generate_insert_statements(
    ...
    print(f"Range de datas: {start_date} até {(start + timedelta(days=num_days-1)).strftime('%Y-%m-%d')}")
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"-- Range de datas: {start_date} a {(start + timedelta(days=num_days-1)).strftime('%Y-%m-%d')}\n")
        ...
        for batch in range(num_batches):
            ...
            print(f"Batch {batch + 1}/{num_batches}: Gerando registros {start_idx + 1:,} a {end_idx:,}...", end="\r")
...
def generate_large_dataset(
    output_file="load-iceberg-large.sql",
    size_gb=10
):
    """
...
Exemplos de uso:
  ...
  python generate_data.py --records 1000000 --days 365 --start-date 2023-01-01
    ...
    parser.add_argument(
        ...
        type=int,
        default=90,
        help="Número de dias no range de datas (padrão: 90)"
    )
    ...
    parser.add_argument(
        "--size-gb",
        type=float,
        help="Gerar dataset baseado no tamanho em GB (ex: 10 para 10GB)"
    )
